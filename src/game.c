...
int weighted_random() {
  int r = rand() % 100;
  if (r < SINGLE_CELL_CHANCE)
    return 1;
  else if (r < DOUBLE_CELL_CHANCE + SINGLE_CELL_CHANCE)
    return 2;
  else if (r < TRIPLE_CELL_CHANCE + DOUBLE_CELL_CHANCE + SINGLE_CELL_CHANCE)
    return 3;
  else
    return 4;
}

// Helper function to latin square for kenken board, (make more randomized in
// the future)
void generate_latin_square(GameState *game) {
  ...
  // Symbol shuffle, ex: for 4x4, symbols = [3, 1, 4, 2], row = [1, 2, 3, 4],
  // now row = [3, 1, 4, 2]
...
void assign_cage_op(GameState *game, Cage *cage) {
  ...
  if (cage->size == 2) {
    ...
    Operation ops[] = {ADD, SUB, MUL,
                       DIV, DIV, DIV}; // Div 3 times to weigh the odds
    rand_op = ops[rand() % 6];
...
void generate_puzzle(GameState *game) {
  ...
  while (assigned_num < game->size * game->size) {
    ...
    if (assigned[rand_x][rand_y])
      continue; // Skip assigned cells
    if (game->cage_count >= MAX_CAGES)
      break;
