...
void init_graphics() {
  // Init color pairs
  init_color(COLOR_MAGENTA, 700, 700,
             700); // Doesn't work on my terminal, maybe on some it will idk
  ...
  for (int i = 0; i < game->size; i++) {
    for (int j = 0; j < game->size; j++) {
      ...
      if (i + 1 < game->size && j + 1 < game->size) {
        ...
        bool hline = (cage->cage_id != bottom->cage_id ||
                      right->cage_id != diag->cage_id);
        bool vline = (cage->cage_id != right->cage_id ||
                      bottom->cage_id != diag->cage_id);
...
/*
        Render board
*/
void render_board(GameState *game, Cursor *cursor, int bh, int bw, int by,
                  int bx) {
  // Draw game box
  ...
  for (int i = 0; i < game->size; i++) {
    for (int j = 0; j < game->size; j++) {
      ...
      if (cursor->row == i && cursor->col == j) {
        pair = 6;
      } else if (cage->cage_id ==
                 game->cells[cursor->row][cursor->col].cage->cage_id) {
        pair = 7;
      } else {
        pair = color;
      }
      ...
      if (is_label) {
        ...
        mvwprintw(board, screen_y, screen_x, "%d%c", cage->target,
                  (char)cage->op);
      ...
      if (val != 0) {
        wattron(board, COLOR_PAIR(pair) | A_BOLD);
        mvwprintw(board, screen_y + CELL_H / 2 - 1, screen_x + CELL_W / 2 - 1,
                  "%d", val);
        wattroff(board, COLOR_PAIR(pair) | A_BOLD);
      } else {
        // Draw notes in 3x3 grid in cell, up to 9 notes shown as small numbers
        int note_positions[MAX_SIZE][2] = {{0, 0}, {0, 2}, {0, 4},
                                           {1, 0}, {1, 2}, {1, 4},
                                           {2, 0}, {2, 2}, {2, 4}};
...
int render_menu(MenuState *menu) {
  ...
  for (int i = 0; i < DIFF_COUNT; i++) {
    ...
    if (is_selected) {
      attron(COLOR_PAIR(diff_colors[i]) | A_BOLD |
             (is_focused ? A_REVERSE : 0));
    } else {
      attron(COLOR_PAIR(5));
    }
    mvprintw(mid_y - 6, mid_x - 3 + diff_offsets[i], "%s%s%s",
             is_selected ? "[" : " ", diff_labels[i], is_selected ? "]" : " ");
...
int handle_menu_input(MenuState *menu, int ch) {
  ...
  case 'k':
    menu->section =
        (menu->section - 1 + MENU_SECTION_COUNT) % MENU_SECTION_COUNT;
    break;
  case KEY_DOWN:
  case 'j':
  ...
  case KEY_LEFT:
  case 'h':
    if (menu->section == MENU_DIFFICULTY)
      menu->diff = (menu->diff - 1 + DIFF_COUNT) % DIFF_COUNT;
    else if (menu->section == MENU_SIZE)
      menu->size = (menu->size - 1 < MIN_SIZE) ? MAX_SIZE : menu->size - 1;
    break;
  case KEY_RIGHT:
  case 'l':
    if (menu->section == MENU_DIFFICULTY)
      menu->diff = (menu->diff + 1) % DIFF_COUNT;
    else if (menu->section == MENU_SIZE)
      menu->size = (menu->size + 1 > MAX_SIZE) ? MIN_SIZE : menu->size + 1;
    break;
  case '\n':
  case KEY_ENTER:
    if (menu->section == MENU_PLAY)
      return 1;
    menu->section = (menu->section + 1) % MENU_SECTION_COUNT;
    break;
  default:
...
void handle_input(GameState *game, Cursor *cursor, InputMode *mode, int ch) {
  ...
  case KEY_UP:
  case 'k':
    cursor->row = (cursor->row - 1 + game->size) % game->size;
    break;
  case KEY_DOWN:
  case 'j':
    cursor->row = (cursor->row + 1) % game->size;
    break;
  case KEY_LEFT:
  case 'h':
    cursor->col = (cursor->col - 1 + game->size) % game->size;
    break;
  case KEY_RIGHT:
  case 'l':
    cursor->col = (cursor->col + 1) % game->size;
    break;
  ...
  // Show cell
  case 'c':
    game->grid[cursor->row][cursor->col] =
        game->solution[cursor->row][cursor->col];
    for (int i = 0; i < game->size; i++) {
      game->notes[cursor->row][cursor->col][i] = 0;
    }
    break;
